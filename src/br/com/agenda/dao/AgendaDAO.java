...
public class AgendaDAO {
    ...
    public boolean deletaAgendaPorId(long id) throws SQLException {
        Agenda ag = new Agenda();
        conn = getConnection();
    ...
    public boolean atualizaAgendaPorId(Agenda ag) throws SQLException {
        conn = getConnection();
        String sql = "UPDATE agendatb SET compromisso = ?, descricao = ?, data = ?, localizacao = ? WHERE id = ?";
        try {
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, ag.getCompromisso());
            stmt.setString(2, ag.getDescricao());
            stmt.setTimestamp(3, Timestamp.valueOf(ag.getData()));
            stmt.setString(4, ag.getLocalizacao());
            stmt.setLong(5, ag.getId());
            stmt.executeUpdate();
            return true;
        } catch(SQLException e){
            Logger.getLogger(AgendaDAO.class.getName()).log(Level.SEVERE, null, e);
